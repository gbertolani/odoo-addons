odoo.define('widget_google_maps_drawing.FieldMapDrawingShape', function (require) {
    ...
    var FieldMapDrawingShape = BasicFields.InputField.extend({
        ...
        _loadDrawingColorPicker: function(){
            if (this.$btnDrawingColorPicker === undefined) {
                this.$btnDrawingClear = $(qweb.render('WidgetDrawing.BtnDrawingColorPicker', {
                    widget: this
                }));
                this.btnColorPicker = Pickr({
                    el: this.$btnDrawingClear,
                    default: '#006ee5',
                    components: {
                        preview: true,
                        opacity: true,
